...
export class RegisterComponent implements OnInit {
    ...
    ngOnInit(): void {
      this.RegisterForm = new FormGroup({
       ...
       password:new FormControl('',[Validators.required, Validators.pattern('^.*(?=.{8,})(?=.*\d)(?=.*[a-z])(?=.*[A-Z])(?=.*[!*@#$%^&+=]).*$'),Validators.minLength(8)]),
        confirmPassword:new FormControl('',[Validators.required, Validators.pattern('^.*(?=.{8,})(?=.*\d)(?=.*[a-z])(?=.*[A-Z])(?=.*[!*@#$%^&+=]).*$'),Validators.minLength(8)])
      })
    }
    password() {
      this.hide = !this.hide;
  ...
  getErrorMessage(inputName:string)
   {
    let len = inputName=="password"||inputName=="confirmPassword"? 8:3;
